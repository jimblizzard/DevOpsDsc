...
Describe 'Web Server E2E' {
    Context 'DNS addressess' {
        It "Should resolve bliztestagent1 to 10.0.0.40" {
            (Resolve-DnsName -Name bliztestagent1.contoso.com -DnsOnly -NoHostsFile).IPAddress | Should be '10.0.0.40' 
        }
        
        It "Should resolve bliztestagent2 to 10.0.0.50" {
            (Resolve-DnsName -Name bliztestagent2.contoso.com -DnsOnly -NoHostsFile).IPAddress | Should be '10.0.0.50' 
        }

        It "Should resolve DNS to bliztestagent1" {
            (Resolve-DnsName -Name dns.contoso.com -Type CNAME -DnsOnly -NoHostsFile).NameHost | Should match 'bliztestagent1' 
        }
    }
    
    Context 'Web server ports' {

        $PortTest = Test-NetConnection -ComputerName bliztestagent2.contoso.com -Port 80
    ...
    Context 'Website content' {
        $WebRequest = Invoke-WebRequest -Uri http://bliztestagent2.contoso.com -UseBasicParsing
